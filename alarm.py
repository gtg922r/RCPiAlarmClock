...
def setupLogger(loggerName):
    '''Setup queue-based logger that logs to queue, and listener the streams from queue'''
...
class RotaryEncoderGPIO:
    ...
    def __init__(self,
        ...
        self.workerThread = threading.Thread(name='Encoder Worker', target=self._workerFunction)
...
# IDEA
        # for invalid state, see which channel fired the interuppt and assume that one went first, and that we just missed the transision!!
        state = self.encoder_state(A, B)
        lastState = self.lastState

        if state == lastState:
            pass
        elif ((lastState == self.encD and state == self.encR) or
              (lastState == self.encR and state == self.encM) or
              (lastState == self.encM and state == self.encL) or
              (lastState == self.encL and state == self.encD)):
            self.encCount += 1
        elif ((lastState == self.encD and state == self.encL) or
              (lastState == self.encL and state == self.encM) or
              (lastState == self.encM and state == self.encR) or
              (lastState == self.encR and state == self.encD)):
            self.encCount -= 1
        else:
            self.log.info("Invalid Transition! Last State: {} Current State: {}".format(
                lastState, state))
            self.invalid = True
        ...
        if state == self.encD:
            if self.invalid:          # Invalid Transition was seen
                ...
                if self.change_callback:
                    threading.Thread(target=self.change_callback, args=(1,)).start()
        ...
        self.log.debug("Change | A: {}  B: {} | State = {} | Count = {:2}".format(A,
                                                                                  B,
                                                                                  state,
                                                                                  self.encCount))
...
class AlarmClock:
    def __init__(self, loggingLevel = logging.INFO):
        ...
        self.log.setLevel(loggingLevel)  # This toggles all the logging in class
    ...
    def scheduleAlarm(self):
        ...
        self.alarmJob = self.scheduler.add_job(self.alarmFunction, 'cron',
                                                hour = self.alarmTime.hour,
                                                minute = self.alarmTime.minute,
                                                day_of_week = daysOfWeek)
        self.log.info("Alarm Scheduled for: {} on {}".format(self.alarmTime, daysOfWeek))
    ...
    def alarmFunction(self):
        ...
        cast = next(cc for cc in chromecasts if cc.device.friendly_name == "Bedroom")
        ...
        mc.play_media('http://ice1.somafm.com/groovesalad-128-aac', 'audio/aac')
...
#                                             'fonts', 'ARCADE.TTF'))                                                 
        ...
        draw.text((10, 20), datetime.datetime.now().strftime('%H:%M:%S'),
                  fill="white", font=font_tall)
        draw.text((6, 49), "Wake up: {:2}:{:02}".format(alarm.alarmTime.hour,
                                                         alarm.alarmTime.minute),
                  fill="white", font=font_med)
        

class MQTTAlarmClockInterface:
    def __init__(self, loggingLevel = logging.INFO):
        self.log = setupLogger("mqttClient")
        self.log.setLevel(loggingLevel)  # This toggles all the logging in class
        self.log.info("Creating mqtt connection...")

        self.client=mqtt.Client()
        self.client.username_pw_set(username='homeassistant', password='homeassistant')
        self.client.on_connect = self._on_connect
        self.client.on_message = self._on_message
        self.client.connect('192.168.86.68')
        self.client.loop_start()

        
        
    def _on_connect(self, client, userdata, flags, rc):
        self.log.info("Connected to mqtt with result code "+str(rc))
...
def main():
    ...
    mqtt_client = MQTTAlarmClockInterface(loggingLevel = logging.INFO)
    ...
    def encoder_change_callback(direction):
        ...
        new_time = (datetime.datetime.combine(dummyDate, alarm.alarmTime) + delta).time()
        alarm.alarmTime = new_time
        #updateOLED(oled_device, alarm) # need to be protected by crtiical block
        
    
    encoder = RotaryEncoderGPIO(pinA, pinB,
                                loggingLevel = logging.WARNING,
...
#            end_time = time.perf_counter()
        ...
        print("Cleaning Up!")
        encoder.cleanup() # Clean-up. Need to find a way to do it external. Maybe with contexts and `with`?
